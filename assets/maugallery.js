(function($) {
  ...
  $.fn.mauGallery.methods = {
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-body">
                            ${
                              navigation
                                ? '<div class="mg-prev" role="button" style="cursor:pointer;position:absolute;top:50%;left:-15px;background:white;"><</div>'
                                : '<span style="display:none;" />'
                            }
                            <img class="lightboxImage img-fluid" alt="Contenu de l'image affichée dans la modale au clique"/>
                            ${
                              navigation
                                ? '<div class="mg-next" role="button" style="cursor:pointer;position:absolute;top:50%;right:-15px;background:white;}">></div>'
                                : '<span style="display:none;" />'
                            }
                        </div>
                    </div>
    ...
    prevImage() {
      ...
      let imagesCollection = $("img.gallery-item").toArray();
      let index = imagesCollection.indexOf(activeImage[0]);
      next = null;
      let prev = imagesCollection[(index - 1 + imagesCollection.length) % imagesCollection.length];
    ...
    nextImage() {
      ...
      let index = imagesCollection.indexOf(activeImage[0]);
      prev = null;
      let next = imagesCollection[(index + 1) % imagesCollection.length];
      $(".lightboxImage").attr("src", $(next).attr("src"));
